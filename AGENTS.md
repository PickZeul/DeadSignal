...
The current confirmed gameplay input mapping is Arrow Keys for Player Movement, Z for Slash, X for Dash, and C for Execute. The previous WASD, J, K, and L gameplay inputs are retired and no longer handled. The future M0 Title input rule is Arrow Keys for Title Menu Navigation and Z for Title Menu Confirm. M0 Title itself is not implemented.

The current minimal foundation is Window + Real-time Loop + Software Framebuffer + Keyboard Input + Runtime Generated Audio + Logical Play Space + Player Display + Player Movement + Eight-Direction Facing + Minimal Slash + Minimal Dash + Minimal Execute + Screen Boundary + Single Test Wall Collision + Static Enemy Display + Prototype Enemy Vision with single-Wall Occlusion + Minimal Detection State.

The final Architecture, integer-scaling/pixel-perfect policy, Window size policy, final Player pixel art, palette, Animation frame specification, actual Wall/Tile specification, need for a general Collision structure, Room structure, final Enemy visual size, Enemy-specific sprite specification, Enemy collision specification, role-specific Enemy design, final Vision/FOV values and representation, final Player Detection geometry rule, Wall Occlusion/LOS design, actual Enemy AI, final Execute reach conversion and presentation, relationship between gameplay width W and visual width, FPS/timing/pacing policy, Audio API, and actual sound specifications remain `미정`. Do not implement the next stage until explicitly requested.
