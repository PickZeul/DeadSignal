...
`P06 Minimal Detection State` is complete. The existing P05 final `playerInVision` result is reused directly as the only Detection input, without duplicating or abstracting the raw FOV, single-Wall Occlusion, or final visibility calculation. The minimal state is stored as Detection progress, lost-sight elapsed time, and an ALERT latch. The state flow is CALM -> SUSPICION -> ALERT. Visible time fills Detection from `0.0` to `1.0` in approximately `3.0 seconds` (`초기 기준`), equivalent to a fill rate of `1/3 per second`. Lost sight preserves the current progress for approximately `0.5 seconds` (`초기 기준`), then decays at the same `1/3 per second` rate, so full decay from `1.0` to `0.0` takes approximately `3.0 seconds` (`초기 기준`). Reacquiring sight during either hold or decay resets only the lost-sight elapsed time and immediately resumes filling from the remaining progress. Progress reaching zero returns to CALM; full Detection enters ALERT. P06 ALERT is latched, has no decay, and cannot return to SUSPICION or CALM.

SUSPICION displays a minimal runtime-generated yellow `?` and ALERT displays a minimal runtime-generated red `!` near the Enemy using direct `3x5` framebuffer pixel patterns. CALM displays no Detection symbol. The existing P05 FOV geometry, color, clipping, and binary visible marker remain unchanged. No Detection bar, percentage UI, FOV color progression, external Asset, generic StateMachine/Manager, Enemy AI, Movement, Chase, Search, or ALERT-release behavior was added.

The measured P06 executable sizes are `15,360 bytes` for MSVC Release Win32/x86 (`+512 bytes` from P05) and `20,480 bytes` for MSVC Release x64 (`+512 bytes` from P05). Both solution platforms passed Release Rebuild and execution validation. Continuous visible Detection reached ALERT in approximately `2.976 seconds` on x86 and `2.978 seconds` on x64. Separate x86 runs validated the `0.5-second` hold, hold/decay reacquisition without progress reset, full decay to CALM, Wall-occluded raw-FOV positions not increasing Detection, angle/range/behind-Enemy rejection, ALERT latching, both procedural symbols, and the existing movement, collision/sliding, Vision/Occlusion, audio trigger, real-time loop, aspect-fit, resize, minimize/maximize, and close paths. No external Library, new import, new source/header, or Release setting change was added.
