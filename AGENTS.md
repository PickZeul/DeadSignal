...
The current minimal foundation is Window + Real-time Loop + Software Framebuffer + Keyboard Input + Runtime Generated Audio + Logical Play Space + Player Display + Player Movement + Screen Boundary + Single Test Wall Collision + Static Enemy Display + Prototype Enemy Vision with single-Wall Occlusion.

The final Architecture, integer-scaling/pixel-perfect policy, Window size policy, final Player pixel art, palette, Animation frame specification, actual Wall/Tile specification, need for a general Collision structure, Room structure, final Enemy visual size, Enemy-specific sprite specification, Enemy collision specification, role-specific Enemy design, final Vision/FOV values and representation, final Player Detection geometry rule, Wall Occlusion/LOS design, actual Enemy AI, relationship between gameplay width W and visual width, FPS/timing/pacing policy, Audio API, and actual sound specifications remain `미정`. Do not implement `P06` until explicitly requested.
